...
export function useAuth() {
  ...
  const logout = useCallback(async () => {
    await signOut({
      fetchOptions: {
        onSuccess: () => {
          // This device may be shared: drop the remembered identity and the
          // local Zero store, then hard-navigate so in-memory state dies with
          // the page instead of leaking into the next session.
          clearIdentity();
